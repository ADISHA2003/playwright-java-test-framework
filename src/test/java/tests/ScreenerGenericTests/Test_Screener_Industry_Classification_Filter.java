...
public class Test_Screener_Industry_Classification_Filter extends BaseUI_Test {

    @Test(description = "As a user i view the results as non zero of every graph filter on filter and data table page along with values in linked tabs",dataProviderClass = utils.ScreenerJsonDataProvider.class,dataProvider = "getIndustryClassificationFilters",groups = {"smoke","regression"})
    public void verifyUserApplySectorTreeFilterAndNavigatesToDataTable(String screenerName,String groupName,String subgroupName,String category,String subCategory,String filterName,String currentFilterType,Boolean isDefault,String api,String filterAPIReference) {
        int position=0;
        String filterPreCondition="Where";
        String screenerTile = "All";

        FilterFormat ff = new FilterFormat(screenerName,screenerTile,filterPreCondition,position,groupName,filterName,currentFilterType,"In",new LinkedList<String>(){{add("Finance -> Investment");}});
